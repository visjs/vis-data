...
describe("DataSet", function () {
  it("should work", function () {
    ...
    assert.deepEqual(data.get().toSorted(sort), [
  ...
  it("add should prevent duplicate ids", function () {
    const dataset = new DataSet([{ id: "duplicate" }]);
    assert.throws(
      function () {
  ...
  describe("min", function () {
    it("finds the minimum value", function () {
      const dataset = new DataSet([{ id: 1 }, { id: 2 }, { id: 3 }]);
      const minValue = dataset.min("id");
  ...
  describe("distinct", function () {
    it("finds distinct values", function () {
      const dataset = new DataSet([{ val: 1 }, { val: 2 }, { val: 3 }]);
      const distinctValues = dataset.distinct("val");
